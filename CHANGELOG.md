...
- **同步页重构为总览同款全高等高左右工作区**：移除多卡片平铺与页面级长滚动，改为左主工作区（变更列表表格内部独立滚动，筛选条/表头/汇总底栏固定吸顶，1440×900 一屏 17+ 行）+ 右操作侧栏（同步面板、回收站、完整日志、飞书侧待处理折叠区）；同步结果报告全宽泊靠等高区下方（自限高 300px），清除后即恢复满屏工作区；窄视口（<lg）优雅降级为纵向堆叠。
- **清理「紧凑/舒适」密度切换**：删除分段按钮、localStorage 持久化与 ChangeItem 舒适分支样式，锁定 34px 高密度紧凑行。

### Fixed（ci）

- **release.yml 双 mac job 并发创建 release 竞态**：arm64/x64 两 job 几乎同时到达 `gh release view || gh release create`，都 view 失败后都 create，后到者撞 `422 already_exists` 在 `set -e` 下挂掉整个 job（v0.3.38 发布实测，定向重跑恢复）。修复：抽取幂等 `ensure_release`（create 失败后复查已存在即视为成功），mac/win job 同款。
...
- **重授权降级用户既有权限（并集重授权策略）**：应用内「开始飞书认证」此前只请求 9 项最小必需集——对同时用 lark-cli 跑其他工作流（im/base/board 等）的用户，补授权会把 token 从几十项**降级到仅 9 项**，破坏其他工具链；且对服务端静默丢弃部分 scope 的旧 token（如手动 `--domain all` 授权产物）只能反复重试同一列表。现发起授权前探测当前已授权 scope，请求集 = 已授权 ∪ 必需（剔除已退役名）：补齐缺失的同时完整保留既有权限；若并集请求被上游以「invalid or malformed scopes」整单拒绝，自动降级用最小必需集重试一次兑底。
- **授权发起失败的真实原因被吞**：lark-cli 在 device authorization 被拒时以 exit 0 返回 `{ok:false,error:{message}}`，旧解析只会报「设备授权响应缺少 device_code」误导排查；现优先透传上游错误信息（如整单 scope 无效）。
- **缺权限提示补充版本指引**：「缺少权限」文案增加「若重新授权后仍缺失，多为 lark-cli 版本过旧（应用凭据未发布该权限时授权端会静默丢弃），可尝试更新 lark-cli 后重试」——覆盖服务端永不补齐场景的自助排查。
...
- **飞书认证卡死（授权后仍提示权限未授权）**：2026-09 曾把交替新名 `docs:document:read` 加入默认权限需求集并 additive 迁移写入所有存量配置；该名已在飞书授权端失效（lark-cli OAuth 应用启用清单 198 项中无此名），带上它发起 Device Flow 会被整单拒绝（「The provided scope list contains invalid or malformed scopes」），用户在授权页确认后仍反复提示「缺少权限」。修复：从默认需求集移除该名（回归 9 项全有效集合）；存量配置加载时一次性剔除并写回（升级即自愈）；发起授权前再兑底过滤（防手改配置回灌）；新旧名等价组保留——旧授权只持 `docx:document:readonly` 的用户不误报、无需重新授权。
- **lark-cli 路径设置项下线（错误路径无法清空）**：设置页「lark-cli 路径（可选）」一旦填入错误路径即无法清空（清空后 undefined 在 JSON 序列化时丢键、服务端浅合并保留旧值），且错误路径永久压制 PATH/常见安装位置自动发现，即使 CLI 已正确安装也无法识别。修复：移除该设置项（UI 与配置模型同步删除）；config.json 存量错误路径在加载时物理清除（版本升级自愈）；PUT 合并点剔除旧版前端缓存回传的废弃键；lark-cli 解析统一走自动发现，部署级逃生口保留 `LARK_CLI_PATH` 环境变量。
...
- **预览浮动图片加载失败（mac/win 同现）**：同步产物文件名含半角括号（如 `③-1(Framwork)分析拆解（确定复杂度）_B63_x.png`）时，预览 inline 解析的 URL 模式 `[^)\n]*` 在文件名内首个 `)` 截断 src，拿到无扩展名的残路径 → 415/404 → 「图片加载失败」。改为允许一层成对括号嵌套（CommonMark 平衡括号规则），无括号 URL 行为不变；CSV 链接跳转同款截断一并修复。另：图片路径反斜杠归一（win 历史产物防御）+ 加载失败占位块显示解析后路径与 HTTP 状态（后续排查可见化）。

### Added（feat）

- **docx 内嵌电子表格展开**：飞书 docx 导出 md 中的 `<sheet sheet-id="…" token="…">` 引用标签此前从未被同步链路处理（仅展开 objType=sheet 整文档型），原样落盘导致预览直接显示裸 XML（实测 30+ 份历史产物含残留）。现同步时逐标签导出子表（复用 csv + 浮动图片全管道）→ 表格重构 → 标签级替换为「## 子表:」段（含 CSV 链接与浮动图片附录）；任一子表失败不推进 synced 基线，下次重试。前端同层兜底：未展开的历史标签优雅降级为提示块（含 sheet-id/token 与重新同步指引），不再露裸 XML。
...
- **本地目录自动命名**：添加飞书根 URL 时本地目录可留空，保存时服务端拉取根节点标题自动生成安全目录名（非法字符清洗/首尾修剪/64 字截断/Windows 保留名回退 wiki-<token>，与手填路径同一安全边界），同批冲突自动加后缀，并在知识库根下立即创建目录；标题拉取失败回退 token 命名，不阻断保存。
- **孤立文件清理**：设置页新增「孤立文件清理」卡片，扫描知识库根目录下与任何同步根（含停用）/自定义归档无对应关系、且含 feishu_sync 头（同步产物铁证，用户自有内容永不误伤）的目录/散档，一键移动到 .trash-bin/orphan-<时间戳>/ 回收站（可恢复，非删除）；服务端实时重扫校验防路径注入，双重越界守卫。

### Fixed（fix）

- **同步根被清理后残留「未分类」无法清理**：移除同步根 URL 时自动级联清除其 DB 数据（documents/sheet_sheets/feishu_pending_items/localDirs，事务内显式级联），响应带清理计数摘要；本地文件不静默删除，由孤立文件清理手动处理。
...
- **整体布局优化（六项）**：①节点树面板空间释放——移除视图切换卡片与说明文字，底部统计/提示压为紧凑单行（辅助提示收 Tooltip），默认宽度 260px→320px（可拖拽 280-560px，双击复位，持久化）；②移除本地视图页签，固定飞书视图；③目录布局固定「镜像标题文件」模式，不再提供切换；④设置页隐藏未完善的「模型与整理」子页入口；⑤总览页底部留白压缩，树/预览/详情三栏吃满可用高度；⑥顶部栏位去重紧凑——TopBar 压至 50px 且去除与状态栏重复的认证/待同步徽标，GlobalStatusBar 压至 ~38px，消除 600px+ 中部空白。
...
- **「在文件夹中打开」二次修复**：上版修错了方向（拼路径/调用链本身正确），真断点是 Electron showItemInFolder 对不存在路径**静默无动作**——local_path 与磁盘不一致（目录重排/手动移动）时点了毫无反应。现 main 进程显式三级降级：文件在→定位选中；父目录在→打开父目录；都不在→返回明确错误供前端 toast 显示。前端同步兼容 legacy 绝对路径形态的 local_path。
- **界面缩放快捷键（高分屏文本模糊缓解）**：Win 125%/150% 非整数缩放下文本光栅化半像素发虚，新增 VSCode 同款缩放：Ctrl+= 放大 / Ctrl+- 缩小 / Ctrl+0 重置（步进 25%，夹 0.25x-4x），右下角 OSD 显示当前倍率，localStorage 持久化重启恢复。微调到整数物理像素倍率可显著改善清晰度。
